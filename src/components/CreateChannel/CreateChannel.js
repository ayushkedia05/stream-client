...
const CreateChannel = ({ onClose, toggleMobile }) => {
  ...
  const createChannel = async () => { 
    ...
    // await conversation.addMembers([{user_id:client.user.id ,channel_role:"channel_moderator"}]);
    // await conversation.addModerators([client.user.id])
    ...
    onClose();
     const channeldata={
      ...
      channeladmin:client.userID
     }
    const getdata=await axios.post('http://localhost:3000/createchannel',channeldata)
      console.log(getdata)
